...
module.exports = async function handler(req, res) {
  ...
  if (ageHours > 48) {
    res.status(410).json({
      error: 'This payment is too old to generate automatically. Email hello@ledgerworks.example and I will send it manually.'
    });
    return;
  }
  ...
  if (kv) {
    try {
      ...
      if (!ok) {
        res.status(409).json({
          error: 'This payment has already been used. If you did not receive your brief, email hello@ledgerworks.example.'
        });
        return;
      }
      claimed = true;
  ...
  if (!answers.idea) {
    await releaseClaim();
    res.status(400).json({ error: 'Could not recover your answers. Email hello@ledgerworks.example and I will generate it manually.' });
    return;
  }
